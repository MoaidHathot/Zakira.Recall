...
internal static class DuckDuckGoHtmlParser
{
    ...
    private static readonly Regex DisplayUrlPattern = new(@"<a[^>]*class=\""[^\""]*\bresult__url\b[^\""]*\""[^>]*>(?<text>.*?)</a>", RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.Compiled);
    private static readonly Regex SnippetPattern = new(@"<a[^>]*class=\""[^\""]*\bresult__snippet\b[^\""]*\""[^>]*>(?<text>.*?)</a>", RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.Compiled);
    ...
    private static IReadOnlyList<string> SplitIntoResultSegments(string html)
    {
        ...
        foreach (Match match in matches)
        {
            ...
            if (!ContainsCssClass(classValue, "result") || !ContainsCssClass(classValue, "web-result"))
            {
                continue;
            }
